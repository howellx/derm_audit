...
class ContextUnet(nn.Module):
    ...
    def forward(self, x, c, t, context_mask):
        # x is (noisy) image, c is binary label, t is timestep
        x = self.init_conv(x)
        down1 = self.down1(x)
...
class DDPM(nn.Module):
    ...
    def forward(self, x, c):
        ...
        # Create noisy image based on diffusion schedule
        x_t = (
            self.sqrtab[_ts, None, None, None] * x
            + self.sqrtmab[_ts, None, None, None] * noise
        )

        # Convert labels to integers and create context mask
        c = (c > 0.5).long()  # Convert probability to binary label
    ...
    def sample(self, n_sample, size, device, guide_w=0.0, condition=None):
        """
        ...
        Args:
            ...
            guide_w: guidance scale
            condition: optional specific condition to generate
        """
        ...
        # Create and prepare context mask
        ...
        for i in range(self.n_T, 0, -1):
            ...
            x_i = x_i_orig.repeat(2, 1, 1, 1)
            t_is = t_is.repeat(2, 1, 1, 1)

            # Optional noise for all but last step
            ...
            # Update sample
            x_i_orig = (
                self.oneover_sqrta[i] * (x_i_orig - eps * self.mab_over_sqrtmab[i])
                + self.sqrt_beta_t[i] * z
            )
            
            # Store intermediate results
            if i % 20 == 0 or i == self.n_T or i < 8:
                x_i_store.append(x_i_orig.detach().cpu().numpy())

        return x_i_orig, x_i_store
...
def train_isic():
    ...
    in_channels = 3
    lrate = 1e-4
    save_interval = 5  # Add save interval
    save_path = "diffusion_checkpoint.pth"  # Add save path
